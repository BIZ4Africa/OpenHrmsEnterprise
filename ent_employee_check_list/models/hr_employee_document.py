...
class HrEmployeeDocument(models.Model):
    """Inherits the model hr.employee.checklist"""
    _inherit = 'hr.employee.document'

    document_name_id = fields.Many2one('employee.checklist',
    ...
    def unlink(self):
        ...
        for result in self:
            if result.document_name_id.document_type == 'entry':
                result.employee_ref.write(
                    {'entry_checklist_ids': [(5, result.document_name_id.id)]})
            if result.document_name_id.document_type == 'exit':
                result.employee_ref.write(
                    {'exit_checklist_ids': [(5, result.document_name_id.id)]})
        res = super().unlink()
        return res
